...
class PDF extends FPDF
{
            ...
            //  $this->Image('..//./uploads/products/' . $content, 45, $img_pos_y,25,20);
                ...
                if ($header_key == 'image') {
                    if (!empty($content)) {
                        $x = $this->GetX();
                        $y = $this->GetY();
                        $this->Image('..//./uploads/products/' . $content, $x + 2, $y + 2, 30, 25);
                    }
                    $content = ''; // important: don't print filename
                }
    ...
    if($type == 'product'){
        ...
        foreach($products as $product){
            ...
            // detect double-quotes and escape any value taht contains them
    ...
    // Supplier Export
    if($type === 'supplier'){
        $stmt = $conn->prepare("SELECT suppliers.id as sid,suppliers.created_at as 'created at',
        users.first_name,users.last_name,suppliers.supplier_location,suppliers.email, suppliers.created_by
    ...
    if($type === 'purchase_orders'){
        $stmt = $conn->prepare("SELECT products.product_name,order_product.id,order_product.quantity_ordered as qty_ordered,order_product.quantity_received as qty_received,
                        order_product.quantity_remaining as qty_remaining,order_product.status,order_product.batch,users.first_name,users.last_name,
                        suppliers.supplier_name,order_product.created_at as 'created at' 
                        FROM order_product
        ...
        $data[] = [
                ...
                // ],
                'qty_ordered' => [
                    'content' => $order_product['qty_ordered'],
                    'align' =>'C'
                ],
                'qty_received' => [
                    'content' => $order_product['qty_received'],
                    'align' =>'C'
                ],
                'qty_remaining' => [
                    'content' => $order_product['qty_remaining'],
                    'align' =>'C'
                ],
                'status' => [
